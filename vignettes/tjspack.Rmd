...
Going forward this package could be expanded to include simpler functions that could flag subjects whose demographic information like race, gender, or sex or even survey questions like medical history change in unexpected ways. Other ways to safeguard longitudinal datasets from data entry errors. I have a lot of experience in lead poisoning surveillance, which as a chronic medical condition has a lot of similarities to a longitudinal dataset when you are talking about tracking patients suffering from lead poisoning. In lead poisoning surveillance there are several case management milestones that need to happen for every confirmed case, namely an educational visit and a risk assessment to discover and then mitigate the hazard. I have an idea for a function that would flag confirmed cases that are yet to achieve these actions in a reasonable window after becoming confirmed, perhaps 180 days. It would be a way to quickly ensure patients that are perhaps on the road to becoming lost or are part of a backlog as not completely forgotten about. It would be quite simple, something along the lines of if system date is greater than 180 days after the confirmatory test date then output that subject. Staying with the lead exposure example, there could be another function that details the level after the risk assessment on a graph. Ideally it would show the levels decreasing after the hazards were identified and removed during the risk assessment. There also could be a function to highlight patients whose recent test results have dropped under the treshold and are perhaps appropriate to confirm former cases. Since lead lingers so long in the body, the retest schedule can be a year between tests, this can make it difficult to associate new results with years old data that subject previously generated. A function that immediately either charted or displayed the test result history in a table would be very useful in maintaining patient records and identifying who still needs resoucres and help. There are many potential avenues to take this package to further improve the benefit it could provide to people looking to safeguard their longitudinal datasets.
